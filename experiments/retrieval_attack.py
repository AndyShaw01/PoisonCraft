...
def setup():
    ...
    parser.add_argument("--exp_mode", type=str, default='main_result', choices=['main_result', 'ablation', 'transfer_attack'], help="The mode of the experiment.")
...
def main(args, writter):
    ...
    with torch.no_grad():
        ...
        all_list = get_adv_suffix_for_main_result(args.domain_list, args.control_str_len_list, args.attack_info, args.retriever)
        ...
        for target_threshold in args.target_threshold:
            ...
            asn = df['ASN'].sum()
            asr = round(asn / len(df) / len(queries), 4)
