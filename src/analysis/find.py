...
def main():
    ...
    for query_id, k_records in data.items():
        ...
        # 检查 k=5 和 k=50 是否成功被投毒
        success_k5 = False
        success_k50 = False
        poisoning_ratio_k5 = 0
        poisoning_ratio_k50 = 0

        record_k5 = k_records[5]
        record_k50 = k_records[50]
        ...
        # 检查 k=50
        injected_adv_k50 = record_k50.get("injected_adv", [])
        if injected_adv_k50:
            output_poison_k50 = record_k50.get("output_poison", "")
            if predictor.predict(output_poison_k50):
                success_k50 = True
                poisoning_ratio_k50 = len(injected_adv_k50) / 50

        # 只有当 k=5 和 k=50 都成功时，继续检查 k=10 和 k=20
        if not (success_k5 and success_k50):
            continue
        ...
        if poisoning_failed_k10 and poisoning_failed_k20:
            result = {
                ...
                "poisoning_ratio_k50": poisoning_ratio_k50
            }
            results.append(result)

    # 输出结果到 JSON 文件
    output_filepath = os.path.join(base_dir, "poisoning_analysis.json")
