...
def get_suffix_db(category_list, threshold_list, attack_info, prefix_mode):
    suffix_db = {}
    suffix_all = {}
    all_list = []
    for category in category_list:
        suffix_db[category] = {}
        # prefix_file_path
        if prefix_mode == 'single_all':
            prefix_file_path = "./Result/attack_prefix/single_category_all_words_5_prefix.csv"
        elif prefix_mode == 'single_filtered':
            prefix_file_path = "./Result/attack_prefix/single_category_filtered_words_5_prefix.csv"
        elif prefix_mode == 'all_all':
            prefix_file_path = "./Result/attack_prefix/all_category_all_words_5_prefix.csv"
        elif prefix_mode == 'all_filtered':
            prefix_file_path = "./Result/attack_prefix/all_category_filtered_words_5_prefix.csv"
        else:
            raise ValueError("Invalid mode")

        # Add prefix to the attack_info
        prefix_file = pd.read_csv(prefix_file_path)
        if prefix_mode == 'single_filtered' or prefix_mode == 'single_all':
            prefix = prefix_file[prefix_file['category'] == category]['most_common_words'].values[0]
        elif prefix_mode == 'all_all' or prefix_mode == 'all_filtered':
            prefix = prefix_file['most_common_words'].values[0]

        attack_info = prefix + ',' + args.attack_info
        for threshold in threshold_list:
            # candidate_file = f'./Results/improve_exp/batch-4/category_{category}/results_top_{threshold}.csv'
            candidate_file = f'./Results_prefix/Results/prefix_{prefix_mode}/category_{category}/results_top_{threshold}.csv'
            try:
...
def main(args):
    result_file = f'Result/cross_attack/prefix_mode/{args.prefix_mode}_0917.csv'
    ...
    if args.evaluate_mode == 'single_category':
        writter.writerow(
            ['category', 'attack_source', 'threshold', 'ASN', 'ASR'])
    elif args.evaluate_mode == 'all_category':
        writter.writerow(
    ...
    with torch.no_grad():
        ...
        attack_db, attack_all, all_list = get_suffix_db(category_list, threshold_list, args.attack_info, args.prefix_mode)



        # Load the ground truth
        if args.evaluate_mode == 'target':
            # Load Queries
            ...
            for target_threshold in args.target_threshold:
                ...
                print("\n")
        elif args.evaluate_mode == 'single_category':
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prefix_mode", choices=['single_all', 'single_filtered', 'all_all', 'all_filtered'], default="single_filtered")
    parser.add_argument("--evaluate_mode", choices=['single_category', 'all_category', 'target'], default="all_category")
