        ...
        for control_str_len in control_str_len_list:
            if aggregate:
                for exp in exp_list:
                    ...
                    # candidate_file = f'./Results_from_A800/part_results/Results/{exp}/batch-4/category_{category}/results_{control_str_len}.csv' # contriever(nq, pr=0.5)
                    candidate_file = f'./Main_Results/hotpotqa_1126/batch-4/domain_{category}/combined_results_{control_str_len}.csv' # contriever(msmarco\hotpotqa, pr=0.5)
                    try:
        ...
        if args.mode == 'all_category_by_block':
            ...
            for target_threshold in args.target_threshold:
                for i in range(len(category_list)):
                    ...
                    # Load Ground Truth
                    ground_truth_path = f'./Datasets/nq/ground_truth_topk_ance/ground_truth_top_{target_threshold}_domain_{category_list[i]}.csv'
                    ground_truth_df = pd.read_csv(ground_truth_path)[f'matched_bar_{target_threshold}']
                    ground_truth = torch.tensor(ground_truth_df, device='cuda:3')
                    # pdb.set_trace()
                    ...
                    for query_block_index in range(num_query_blocks):
                        ...
                        for block_index in tqdm(range(num_blocks), desc="Processing Blocks"):
                            ...
                            # cosine similarity: queries_embedding and attack_embedding
                            if 'ance' in args.model_path:
                                queries_embedding = queries_embedding / queries_embedding.norm(dim=1, keepdim=True)  # [499, 768]
                                attack_embedding = attack_embedding / attack_embedding.norm(dim=1, keepdim=True)    # [2048, 768]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_threshold", type=str, default=[19])
    ...
    parser.add_argument("--model_path", type=str, default="/data1/shaoyangguang/offline_model/ance")
