...
def main(args):
    ...
    with torch.no_grad():
        ...
        if args.mode == 'target':
            ...
            # for target_threshold in ar gs.target_threshold:
            target_threshold = 4
            category_list_i = 12
            # Load Queries
            queries_path = args.queries_folder + f"/category_{category_list_i}.jsonl"
            df_queries = pd.read_json(queries_path, lines=True)
            queries = df_queries['text'].tolist()
            queries_embedding = embedding_model(queries)
            ...
            ground_truth_path = f'./Dataset/nq/ground_truth_test_recheck_0905/ground_truth_top_{target_threshold}_category_{category_list_i}.csv'
            ...
            # Identify which queries are "jailbroken" (similarity > ground_truth)
            jailbroken_mask = (similarity > ground_truth).any(dim=1)

            # Get the jailbroken queries
            # jailbroken_queries = [queries[i] for i in range(len(queries)) if jailbroken_mask[i]]
            

            # Save jailbroken queries to a CSV file
            # jailbroken_df = pd.DataFrame(jailbroken_queries, columns=['jailbroken_queries'])
            # pdb.set_trace()
            # jailbroken_df.to_csv(f'./Dataset/case_study/case_study.csv', index=False)

            df_jailbroken_queries = df_queries[jailbroken_mask.cpu().numpy()]
            jailbroken_jsonl_path = "./Dataset/case_study/case_study.jsonl"
            df_jailbroken_queries.to_json(jailbroken_jsonl_path, orient='records', lines=True)
